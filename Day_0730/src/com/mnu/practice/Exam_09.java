...
public class Exam_09 {
	...
	public static void main(String[] args) throws Exception{
		...
		while(sc.hasNext()) {
			...
			stu1.setBun(Integer.parseInt(data[0]));
			stu1.setName(data[1]);
			stu1.setKor(Integer.parseInt(data[2]));
			stu1.setEng(Integer.parseInt(data[3]));
			stu1.setMath(Integer.parseInt(data[4]));
			//중복 검사
			boolean bool = false;
			for(int x=0;x<stu.size(); x++) {
				if(stu1.getBun()==stu.get(x).getBun()) {
